# Test CRD Register configmapgenerator


This folder demonstrates how to use configmapgenerator Kustomize Transformer
...
### Preparation Step Env File devops.env

<!-- @createOther0 @test -->
```bash
cat <<'EOF' >${DEMO_HOME}/devops.env
SERVICE_PORT=32
EOF
```


### Preparation Step Env File uxstream.env

<!-- @createOther1 @test -->
```bash
cat <<'EOF' >${DEMO_HOME}/uxteam.env
COLOR=red
EOF
```
...
kind: ConfigMap
metadata:
  name: configmapgenerator-kc588km8h4
---
apiVersion: my.org/v1alpha1
kind: MyCRD
metadata:

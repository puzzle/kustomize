...
func (o *nameReferenceTransformer) Transform(m resmap.ResMap) error {
	...
	for _, referrer := range m.Resources() {
		...
		byFieldPath := builtinconfig.NewFieldPathMapFromSlice(o.backRefs, referrer.OrgId().Gvk)
		...
		for fieldPath, targets := range byFieldPath {
			pathSlice := types.FieldSpec{Path: fieldPath}.PathSlice()
		...
		case []interface{}:
			for idx, item := range thing {
				...
				case string:
					...
					newName, err := o.getSimpleNameField(value, referrer, targets,
						referralCandidates, referralCandidates.Resources())
					if err != nil {
						return nil, err
					}
				...
				case map[string]interface{}:
					...
					newMap, err := o.getNameAndNsStruct(value, referrer, targets,
						referralCandidates)
					if err != nil {
						return nil, err
					}

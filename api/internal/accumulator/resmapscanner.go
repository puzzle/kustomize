...
var KindRegistry = map[string]resid.Gvk{}
...
func NewResMapScanner(userVars types.VarSet, userRefs types.FsSlice) *ResMapScanner {
...
// This allows the user to not have to do that manually.
func (rv *ResMapScanner) DiscoveredConfig() *builtinconfig.TransformerConfig {
	return &builtinconfig.TransformerConfig{
...
func buildFieldSpec(id resid.ResId, pathSlice []string) *types.FieldSpec {
	// varReference paths use / as separator.
	path := strings.Join(pathSlice, Slash)

	return &types.FieldSpec{
		Gvk:  resid.FromKind(id.Kind),
		Path: path,
	}
}

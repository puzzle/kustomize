...
func (ra *ResAccumulator) MergeAutoConfig() error {
	...
	scanner := NewResMapScanner(ra.varSet, ra.tConfig.VarReference)
...
func (ra *ResAccumulator) findVarValueFromResources(v types.Var, varMap *VarMap) error {
	foundCount := 0
...
func (ra *ResAccumulator) makeVarReplacementMap() (VarMap, error) {
	result := NewEmptyVarMap()
